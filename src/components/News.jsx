...
const News = ({ simplified }) => {
  ...
  const { data: cryptoList } = useGetCoinsQuery(10)
      ...
      {!simplified && (
        <Col span={24}>
          ...
          >
            ...
            {cryptoList?.map((coin) => <Option value={coin.id}>{coin.id}</Option>)}
          </Select>
        </Col>
      )}
        ...
        <Col xs={24} sm={12} lg={8} key={i}>
          <Card hoverable className='news-card'>
            <a href={news.url} target="_blank" rel='noreferrer'>
              ...
              <p>
                {news.description.description > 100 ? `${news.description.substring(0, 100)}...` : news.description }
              </p>
